...
* 이전 장의 끝에서 `AVCaptureVideoDataOutput`에 의해 캡처 된 CvPixelBuffer 객체를 OpenGL ES 텍스처로 렌더링했었다. 그러나 `AVCaptureVideoDataOutput`의 단점은 `AVCaptureMovieFileOutput`이 출력을 기록한다는 것이다.
* iOS 7부터 내장 된 카메라 앱을 사용하면 정지 이미지를 찍을 때 필터를 적용 할 수 있다. 이 필터는 Core Image 프레임 워크에서 찾을 수 있으며 대부분의 최신 세대 iOS 기기에서 실행할 때 앱에 실시간 동영상 효과를 추가하는데 사용할 수 있다.
* 애플의 `RosyWriter`라는 샘플앱을 보면 데이터에 대해 정교하게 처리하기 위한 별도의 대기열을 사용하는 방법과 `CMSampleBuffers`를 효율적으로 처리하기위한 몇 가지 고급 성능 옵션을 알 수 있다.

### Summary
이 장에서는 `AVAssetReader` 및 `AVAssetWriter`가 제공하는 강력한 기능에 대해 배웠습니다. 이 클래스는 AV Foundation에서 미디어를 처리하는 가장 낮은 수준의 방법이며 많은 고급 사용 사례에서 매우 중요한 역할을합니다. CMSampleBuffer 레벨에서 미디어 작업을하면 일정 수준의 복잡성이 드러나지만 프레임 워크의 고수준 기능만으로는 얻을 수없는 고급 기능을 구현할 수도 있습니다. AVAssetReader 및 AVAssetWriter을 자유자재로 다루는데 다소 시간이 걸릴 수 있지만이 이 챕터에서 다루는 주제 및 샘플 프로젝트는 자신의 응용 프로그램에서 빌드 할 수있는 유용한 토대를 제공합니다.